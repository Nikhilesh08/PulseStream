...
export const UserPanel: React.FC<UserPanelProps> = ({
  ...
  // 🛡️ PROTECTED LOGIN: Requires secret passkey before logging into Master Test User!
  const handleMasterTestLogin = async () => {
    setError("");
    const enteredKey = window.prompt(
      "🔐 Enter Master Admin Pass Key to unlock:",
    );

    if (enteredKey === null) return;
    if (enteredKey !== "Nikhilesh@32") {
      setError("❌ Access Denied: Invalid Master Pass Key!");
      return;
    }

    setEmail("test@pulsestream.io");
    setPassword("test1234");
    try {
      const res = await loginUser("test@pulsestream.io", "test1234");
      localStorage.setItem("token", res.data.token);
      setCurrentUser(res.data.user);
    } catch (err) {
      setError("Master test user not seeded yet. Restart backend server!");
    }
  };
  ...
  const handleToggle = async (
    ...
    // 🐛 THE FIX: Safely fallback to an empty array if subscriptions is undefined
    ...
    try {
      ...
      setCurrentUser(previousUser); // Rollback optimistic UI on failure
    }
  };
    ...
    return (
      <div className="max-w-md mx-auto bg-white p-8 rounded-2xl shadow-xl border border-slate-200 mt-10 animate-fadeIn">
        <h2 className="text-2xl font-black text-slate-800 text-center flex items-center justify-center">
          ...
          {isSignup ? "Create Account" : "Sign In to Watchlist"}
        </h2>
        ...
        {error && (
          <div className="mt-4 p-3 bg-rose-50 text-rose-600 text-xs rounded-lg border border-rose-200 font-bold text-center animate-shake">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleAuth} className="mt-6 space-y-4">
          {isSignup && (
            <div>
              ...
              <input
                ...
                className="w-full mt-1 px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-lg text-sm shadow-md transition-all disabled:opacity-50"
          >
        ...
        <div className="mt-6 pt-6 border-t border-slate-200">
          <button
            onClick={handleMasterTestLogin}
            type="button"
            className="w-full py-2.5 bg-slate-900 hover:bg-slate-800 text-emerald-400 font-mono font-bold rounded-lg text-xs border border-slate-700 flex items-center justify-center shadow transition-all active:scale-95"
          >
            <Lock className="h-3.5 w-3.5 mr-2 text-rose-400 animate-pulse" />
            Protected Login: Master Test User
          </button>
        </div>
